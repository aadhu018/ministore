...
export default function CartPage() {
    ...
    async function createCheckout() {
        try {
            ...
            console.log('Error creating checkout', err.messgge)
        }
    }

    // Total Cost of items Cart
    const totalCost = Object.keys(cart).reduce((acc, curr, currIndex) => {
        const costForProduct = cart[curr].quantity * cart[curr].prices[0].unit_amount
        const total = acc + costForProduct
        return total
    }, 0)
